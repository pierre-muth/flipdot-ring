...
#define DATAOUTPIN  D10
#define CLOCKPIN    D8
#define DATAINPIN   D9
// #define DEBUG

static const int spiClkSpeed = 100000; 
static const int CHARWIDTH = 6; // 5 pixels for character + 1 pixel for spacing

SPIClass * spi = NULL; //uninitalised pointers to SPI object
int xShift=0;
...
DotFlippersMatrix::DotFlippersMatrix(int16_t w, int16_t h) : 
    ...
    spi->begin(CLOCKPIN, DATAINPIN, DATAOUTPIN); //SCLK, MISO, MOSI, SS
...
void DotFlippersMatrix::display() {
    ...
    // transmit
    spi->beginTransaction(SPISettings(spiClkSpeed, MSBFIRST, SPI_MODE0));
    spi->transferBytes(flipdotBuffer , NULL, WIDTH);
    spi->endTransaction();
